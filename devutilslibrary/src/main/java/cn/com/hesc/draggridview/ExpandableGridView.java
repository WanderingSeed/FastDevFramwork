 ...
 * ClassName: ExpandableGridView
 * Description: 点击产生二级菜单的gridview，适合全屏的gridview
 * Author: liujunlin
...
public class ExpandableGridView extends GridView {
    ...
    private int column = 3;//竖排时默认的列数
    private OnExpandItemClickListener mListener;
    private int expandableGridViewBg = Color.WHITE;//默认显示白色
    ...
    /**
     * 设置二级菜单的背景色
     * @param expandableGridViewBg
     */
    public void setExpandableGridViewBg(int expandableGridViewBg) {
        this.expandableGridViewBg = expandableGridViewBg;
    }
    ...
    public void expandGridViewAtView(View clickedView, final BaseAdapter expandGridAdapter,boolean isHorizontal){
        ...
        int dd = expandGridAdapter.getCount()%column==0?expandGridAdapter.getCount()/column:expandGridAdapter.getCount()/column+1;
        int middleViewHeight = isHorizontal?clickedView.getHeight()+20:
                clickedView.getHeight()*(dd)+20;// height of middle sub grid view
        ...
        Bitmap bitmapDown = Bitmap.createBitmap(bitmap, 0, downY>=bitmap.getHeight()?downY-1:downY, bitmap.getWidth(), heightDown<=0?1:heightDown);
        ...
        LayoutParams gridParams = new LayoutParams(isHorizontal?count*(getColumnWidth()+hSpace):ViewGroup.LayoutParams.MATCH_PARENT,
                isHorizontal?ViewGroup.LayoutParams.WRAP_CONTENT:middleViewHeight);
        gridView.setLayoutParams(gridParams);
        gridView.setColumnWidth(getColumnWidth());
        gridView.setHorizontalSpacing(hSpace);
        gridView.setVerticalSpacing(vSpace);
        gridView.setNumColumns(isHorizontal?count:column);
        gridView.setPadding(hSpace, vSpace, hSpace, vSpace);
        gridView.setAdapter(expandGridAdapter);
        gridView.setBackgroundColor(expandableGridViewBg);
        ...
        circle.getPaint().setColor(expandableGridViewBg);
        ...
        middleView.setBackgroundColor(expandableGridViewBg);

        // 6. Add the up/middle/down sub views to the cover layer
        mCoverView.addView(imageViewUp);
        mCoverView.addView(isHorizontal?middleView:linearLayout);
